...
def get_delist_data(delist_date=None, url=f'http://{host}:{port}/tqmain/equity_header', wind_code=None):
    ...
    params['fields'] = 'wind_code,delist_date'   # 摘牌日期, 公司股票终止上市并摘牌
    df = get_data(params=params, url=url)
    return df


def get_ev_data(start_date: str, end_date: str, url=f'http://{host}:{port}/tqmain/equity_factor_daily', wind_code=None) -> pd.DataFrame:
    params = {}
    params['wind_code'] = wind_code
    params['start_date'] = start_date
    params['end_date'] = end_date
    params['fields'] = 'trading_date,wind_code,ev'
    df = get_data(params=params, url=url)
    df.sort_values(by='trading_date', inplace=True)
    df = df.reset_index()
    if 'index' in df.columns:
...
def get_industry_data(start_date: str, end_date: str, url=f'http://{host}:{port}/tqmain/equity_industry_daily', wind_code=None) -> pd.DataFrame:
    params = {}
    params['wind_code'] = wind_code
    params['start_date'] = start_date
    params['end_date'] = end_date
    params['fields'] = 'trading_date,wind_code,industry_sw'
    df = get_data(params=params, url=url)
    df.sort_values(by='trading_date', inplace=True)
    df = df.reset_index()
    if 'index' in df.columns:
...
if __name__ == '__main__':
    ...
    get_ev_data(trading_list[0], trading_list[-1]).to_csv("ev_data.csv", header=True, index=True)
    print("Finish!")

    get_industry_data(trading_list[0], trading_list[-1]).to_csv("industry_data.csv", header=True, index=True)
    print("Finish!")
